...
text_speech.say("This is a test")
...
signed_words = ["", "", ""]
# Initializing Pygame window + bar graph settings
pygame.init()
width, height = 900, 830
window = pygame.display.set_mode((width, height))
pygame.display.set_caption('Prediction Analysis')
bar_height = 15
barsList = []
xInitial =  50
yInitial = 410
predictionConfidence = 1.00
# 1: blue, 2: green, 3: bright yellow, 4: yellow, 5: red, 6: pink
...
while running:
    ...
    imgWhite = [0]
    prediction = [0 for i in range(26)]
    lmx = None
    index = -1
    ...
    if hands:
        ...
        if aspectRatio <= 1:
            ...
            prediction, index = classifier.getPrediction(imgWhite)
            print(labels[index])
        predictionConfidence = str(round((prediction[index]/sum(prediction))*100, 2)) +"%"
        print(predictionConfidence)
        landmarks = hand["lmList"]  # Get landmarks
        lmx = []
        lmy = []
        for lm in landmarks:
            lmx.append(lm[0])
